...
class YouTubeChannel(Channel):
    ...
    def check(self, config=None):
        # Run yt-dlp --version to distinguish missing, broken venv, and runtime failures
        ...
        # yt-dlp itself is healthy; JS runtime/transcription checks only affect ok/warn status
        self.active_backend = "yt-dlp"
        # Check JS runtime
        has_js = shutil.which("deno") or shutil.which("node")
        if not has_js:
            return "warn", (
                "yt-dlp is installed but a required YouTube JS runtime is missing.\n"
            ...
            if not _has_js_runtime_config(ytdlp_config):
                ...
                if version is None:
                    return "warn", (
                        ...
                        "Upgrade first and rerun Doctor:\n"
                        f"  {_YTDLP_UPGRADE_COMMAND}"
                    )
                if version < _JS_RUNTIMES_SUPPORTED_FROM:
                    return "warn", (
                        "yt-dlp is too old to support JS runtime configuration. Upgrade first and rerun Doctor:\n"
                        f"  {_YTDLP_UPGRADE_COMMAND}"
                    )
                return "warn", (
