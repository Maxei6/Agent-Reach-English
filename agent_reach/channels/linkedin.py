    ...
    def check(self, config=None):
        ...
        if inspection.server_names & _LINKEDIN_SERVER_NAMES:
            ...
            return "warn", (
                ...
                "service to verify connectivity; configuration alone does not prove full availability."
            )
        if inspection.imports_unchecked:
            return "warn", (
                "LinkedIn MCP was not found in local mcporter config; editor "
                "imports are enabled, and Doctor did not expand them to avoid widening credential reads. Current status is unverified."
            )
        return "off", (
