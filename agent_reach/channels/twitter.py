...
class TwitterChannel(Channel):
    ...
    def check(self, config=None):
        ...
        Same two-stage selection as other multi-backend channels: collect all candidate states; first ok wins;
        only use the first warning if no backend is ok, otherwise an installed-but-not-authenticated twitter-cli
        would incorrectly block a later fully working OpenCLI backend.
        """
        ...
        for backend in self.ordered_backends(config):
            ...
            if result is None:
                continue  # not installed; exclude from candidates
    ...
    def _check_twitter_cli(self, config=None):
        ...
        if auth_token and ct0:
            return "warn", (
                "twitter-cli is installed and explicit Cookie-Editor credentials are configured; "
                "Doctor does not run `twitter status` because upstream may "
                "fall back to reading browser cookies when verification fails. Verify manually only with explicit user consent."
            )
        return "warn", (
            "twitter-cli is installed but complete explicit credentials were not detected. Use Cookie-Editor "
    ...
    def _check_opencli(self):
        ...
        if st.ready:
            return "warn", (
                "OpenCLI bridge is connected, but Twitter/X login state and live commands were not verified; "
    ...
    def _check_bird(self):
        ...
        for cmd in ("bird", "birdx"):
            ...
            if os.environ.get("AUTH_TOKEN") and os.environ.get("CT0"):
                return (
                    "warn",
                    f"{cmd} is installed and explicit environment credentials exist; to avoid upstream "
                    "browser-cookie fallback, Doctor does not run `check`; live status is unverified.",
                )
            return "warn", (
                f"{cmd} is installed but explicit AUTH_TOKEN/CT0 were not detected; "
                "use only credentials manually exported by the user."
            )
        return None
