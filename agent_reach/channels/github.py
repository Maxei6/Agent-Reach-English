...
def _saved_github_host_configured() -> bool:
    ...
    try:
        ...
        raise GitHubConfigError("gh hosts.yml cannot be read safely") from exc
    if raw is None:
        return False
    try:
...
def _explicit_github_credentials(config) -> bool:
    ...
    try:
        ...
        raise GitHubConfigError("Agent Reach GitHub configuration cannot be read") from exc
...
class GitHubChannel(Channel):
    ...
    def check(self, config=None):
        ...
        if probe.status == "broken":
            return "error", (
                "gh exists but cannot run; the installation is broken. Reinstall to fix:\n"
                "  brew reinstall gh\n"
        ...
        try:
            ...
            return "warn", (
                f"gh CLI runs, but authentication configuration cannot be verified safely: {exc}。"
                "Doctor does not run `gh auth status` because it may write a device ID; current auth status is unverified."
            )

        if configured:
            return "warn", (
                "gh CLI runs and explicit authentication configuration was detected; Doctor does not run"
                " `gh auth status` because it may write a device ID, so live auth was not verified and the channel is not marked available."
            )
        return "warn", (
            "gh CLI runs, but no explicit authentication configuration was detected. Run `gh auth login` "
