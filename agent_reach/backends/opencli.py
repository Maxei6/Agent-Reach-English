...
def opencli_status(timeout: int = 10) -> OpenCLIStatus:
    ...
    if not version_probe.ok:
        return OpenCLIStatus(
            ...
            broken=True,
            hint=(
                "opencli exists but cannot execute (broken Node environment). Reinstall:\n"
    ...
    if not st.extension_connected:
        ...
        if st.extension_installed:
            st.hint = (
                ...
                "disk files alone do not prove it is loaded or enabled.\n"
                "  Open the browser extensions page, confirm OpenCLI is enabled, then run an opencli command to verify"
            )
        elif st.unpacked_extension_files:
            st.hint = (
                "Detected ~/.opencli/extension/ source files, but their presence does not prove they are "
                "loaded as an unpacked extension in Chrome/Edge.\n"
                "  Load and enable that directory in the browser extensions page, then run an opencli command to verify"
            )
        else:
            st.hint = (
                ...
                "  2. Keep the browser open, then run an opencli command to verify"
            )
    return st
...
def opencli_summary(st: OpenCLIStatus) -> str:
    ...
    if st.extension_installed:
        return "OpenCLI is installed; extension files were detected but it is not connected (loaded state unverified)"
    if st.unpacked_extension_files:
        return "OpenCLI is installed; unpacked source files were detected but browser loading is unverified"
    if st.daemon_running:
        return "OpenCLI is installed and waiting for the Chrome extension"
    return "OpenCLI is installed (daemon not running; starts on demand; Chrome extension required)"
