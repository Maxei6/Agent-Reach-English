...
def format_report(results: Dict[str, dict]) -> str:
    ...
    lines.append("Legend: [green]✅[/green] available  [yellow][!][/yellow] installed but needs setup/login  [red][X][/red] not installed")
    ...
    # Tier 0 — zero config
    lines.append("")
    lines.append("[bold]✅ Ready with zero configuration:[/bold]")
    ...
    if all_inactive:
        ...
        lines.append(
            f"There are {len(names)} optional channels that can be enabled ({', '.join(names)})，"
            "tell your agent to install the required channel."
        )
