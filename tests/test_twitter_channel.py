...
def test_saved_credentials_are_recognised_without_starting_upstream(
    ...
    assert status == "warn"
    assert "are configured" in message
...
def test_bird_with_explicit_env_remains_unverified(monkeypatch):
    ...
    assert status == "warn"
    assert "live status is unverified" in message
    assert channel.active_backend is None
...
def test_bird_without_explicit_env_is_warn(monkeypatch):
    ...
    assert status == "warn"
    assert "were not detected" in message
    assert channel.active_backend is None
...
def test_opencli_bridge_ready_is_unverified_for_twitter():
    ...
    assert "login state and live commands were not verified" in message
...
def test_verified_backend_result_wins_over_unverified_twitter_cli():
    ...
    with patch.object(
        ...
        TwitterChannel,
        "_check_opencli",
        return_value=("ok", "OpenCLI ready"),
    ), patch.object(TwitterChannel, "_check_bird", return_value=None):
        status, message = channel.check()

    assert status == "ok"
    assert message == "OpenCLI ready"
...
def test_all_warn_returns_first_warning_without_active_backend():
    ...
    with patch.object(
        ...
        TwitterChannel,
        "_check_opencli",
        return_value=("warn", "extension disconnected"),
    ), patch.object(TwitterChannel, "_check_bird", return_value=None):
        status, message = channel.check()
